...
## 2026-07-11 — 암호화 백업 Google Drive 보관

- 평문 표준 백업 압축 파일은 DB dump와 env secret을 포함하므로 Google Drive에 직접 업로드하지 않았다.
- 복구용 암호 파일을 로컬 visualizations 경로에 생성하고, 표준 백업 압축 파일을 GnuPG AES256 대칭 암호화로 `20260711-183556.tar.gz.gpg` 파일로 만들었다.
- 암호화 파일은 임시 복호화 후 원본 tar.gz 해시와 비교하고, tar 목록 확인까지 통과했다.
- 암호화된 백업 파일과 `.sha256` 파일만 `Gdrive:LearnCosmos/backups/server-standard/20260711-183556/`에 업로드했다.
- 복구용 암호 파일은 Google Drive에 업로드하지 않았으며, 서버 삭제 전 별도로 내려받아 안전하게 보관해야 한다.
