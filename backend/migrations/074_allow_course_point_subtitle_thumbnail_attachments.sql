...
ALTER TABLE course_point_attachments
  ...
  CHECK (attachment_type IN ('image', 'file', 'link', 'code', 'other', 'video', 'subtitle', 'thumbnail'));

CREATE UNIQUE INDEX IF NOT EXISTS idx_course_point_research_material_subtitle_one
  ON course_point_attachments(course_point_id)
  WHERE source_context = 'research_material'
    AND attachment_type = 'subtitle';

CREATE UNIQUE INDEX IF NOT EXISTS idx_course_point_research_material_thumbnail_one
  ON course_point_attachments(course_point_id)
  WHERE source_context = 'research_material'
